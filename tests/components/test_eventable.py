...
from tarchia.models import TableCatalogEntry, Schema, Column

def test_subscribe():
    eventable = TableCatalogEntry(
        name="test", steward="test", owner="test", table_id="test", location="test", partitioning=[], last_updated_ms=0, permissions=[], visibility="PUBLIC", current_schema=Schema(columns=[])
    )
...
def test_trigger_event():
    ...
    with patch('requests.post') as mock_post:
        eventable.trigger_event('NEW_COMMIT', {'table_id': '123', 'name': 'ExampleTable'})
        assert mock_post.called
        mock_post.assert_called_with('http://example.com/webhook_created', json={'table_id': '123', 'name': 'ExampleTable'}, timeout=10)
...
def test_subscribe_not_supported_event():
    ...
    with pytest.raises(ValueError):
        eventable.subscribe(user='user1', event='unsupported_event', url='http://example.com/webhook')
...
def test_notify_subscribers():
    ...
    with patch.object(eventable, '_send_request_with_retries') as mock_send_request:
        ...
        mock_send_request.assert_called_once_with('http://example.com/webhook', {'table_id': '123', 'name': 'ExampleTable'})
