...
"""
Debug Import Customization
-------------------------------------
This module provides custom import mechanics for modules.
...
class DebuggingImportFinder(importlib.abc.MetaPathFinder):
    ...
    def find_spec(self, fullname, path, target=None):
        if not (fullname.lower().startswith(("opteryx", "orso", "tarchia"))):
            return None
        ...
        if spec.loader and isinstance(spec.loader, importlib.abc.SourceLoader):
            spec.loader = DebuggingImportLoader(spec.loader)
